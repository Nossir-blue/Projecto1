...
{
    public partial class UC_CustumerRegistration : UserControl
    {
        function fn = new function();
        String query;
        public UC_CustumerRegistration()
        {
            InitializeComponent();
        }
        ...
        private void btnAlloteRoom_Click(object sender, EventArgs e)
        {
            if(txtName.Text !="" && txtContact.Text !="" && txtNationality.Text!="" && txtGender.Text !="" && txtDob.Text !="" && txtIdProof.Text !="" && txtAddress.Text !="" && txtCheckIn.Text !="" && txtPrice.Text !="")
            {
                ...
                query = "insert into custumer (cname,mobile,nationality,gender,dob,idproof,addres,checkin,roomid) values ('"+name+"',"+mobile+",'"+national+"','"+gender+"','"+dob+"','"+idproof+"','"+address+"','"+checkin+"',"+rid+") update rooms set booked = 'YES' where roomNo = '"+txtRoomNo.Text+"'";
                fn.setData(query, "Quarto Nº "+txtRoomNo.Text+" \nAtribuição feita com sucesso");
                clearAll();
            }
            else
            {
        ...
        private void UC_CustumerRegistration_Leave(object sender, EventArgs e)
        {
            clearAll();
        }
    }
